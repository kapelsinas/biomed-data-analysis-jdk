...
```{r Library preparations, warning=FALSE, include=FALSE}
library(pacman)
p_load(Signac, dplyr, heatmaply, reshape2, ggplot2, data.table, matrixStats,
       WGCNA, ggpubr, tidyverse, palmerpenguins, Rtsne,
       methylclockData, methylclock, Signac, dplyr, heatmaply, reshape2, ggplot2, data.table, matrixStats,
...
df <- data.frame(x = age.t1.proximal$Horvath, y = info.t1.proximal$age, aspirin = info.t1.proximal$aspirin)
my.formula <- y ~ x
yy <- "Chronological Age"
  p1 <- ggplot2::ggplot(data = df, ggplot2::aes(x = x, y = y)) + 
...
df <- data.frame(x = age.t2.proximal$Horvath, y = info.t2.proximal$age, aspirin = info.t1.proximal$aspirin)
my.formula <- y ~ x
yy <- "Chronological Age"
  p2 <- ggplot2::ggplot(data = df, ggplot2::aes(x = x, y = y)) + 
    ggplot2::geom_smooth(method = "lm", se = FALSE, color = "black", 
      formula = my.formula) + ggplot2::xlab("DNA Methylation Age") + 
    ggplot2::ylab(yy) + ggplot2::ggtitle(tit) + 
    ggplot2::aes(x, y, colour = aspirin) + ggplot2::geom_point()
