...
if (!file.exists("objects.Rdata")) {
  ...
  if (!is.null(rgSet)) {
    ...
    #     - visas vertes kurių p-vertė didesnė už 0.01 laikykite "blogomis"
    badPValues <- colMeans(detP) > 0.01
    keep <- colMeans(detP) < 0.01
    
    #     - iš RGChannelSet objekto išmeskite visus mėginius (stulpelius), kurie turi daugiau
    #       nei 1% "blogų" detection p reikšmių.
    #     - PAGALBA: iš RGChannelSet objekto reikšmes išmesti galima taip, kaip ir iš
    #       matricos: `myData[,-1]` - išmestų pirmą mėginį (stulpelį)
    rgSet <- rgSet[, keep]
    data <- data[keep, ]
    detP <- detP[, keep]
  }
...
#     su funkcija getSex()).
#     Vien moterų mėginiai mūsų duomenyse
####################################################################################################
