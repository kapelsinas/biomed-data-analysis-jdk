...
gc()
#load pData and annottation 
load("/home/enjoy/Desktop/pData.Rdata")
load("/home/enjoy/Desktop/annotation.Rdata")

#load beta
beta_clean <- readRDS("/home/enjoy/Desktop/dat8.Rds")

col_Beta <- colnames(beta_clean)
row_Beta <- rownames(beta_clean)
...
# unique(annotf$Relation_to_Island) # OpenSea" "Island"  "N_Shore" "N_Shelf" "S_Shore" "S_Shelf"
...
Beta_values <- c(b_island,b_openSea,b_n_shore,b_s_shore ,b_n_shelf, b_s_shelf)

saveRDS(Beta_values, file = "beta_values.rds")
name <- c(rep("Island",length(b_island)), rep("Opensea",length(b_openSea)),
rep("s_shore",length(b_s_shore)),rep("n_shore",length(b_n_shore)), rep("n_shelf",length(b_n_shelf)), rep("s_shelf", length(b_s_shelf)))

saveRDS(name, file = "name.rds")

df <- data.frame(Beta_values, name)

saveRDS(df, "df.rds")

library("ggplot2")

qc_plot <- ggplot(df, aes(x = Beta_values, colour = name)) + geom_density(lwd = 1.2, linetype = 1) + xlim(-0.05,1.0) + xlab("Beta reikšmė") + ylab("Tankis") + 
ggtitle("Metilinimo lygio pasiskirstymas")
