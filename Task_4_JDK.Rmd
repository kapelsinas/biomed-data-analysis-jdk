...
### *Volcano* grafikas visų chromosomų efektų dydžiams ir *p* vertėms atvaizduoti
Žemiau vaizduojamas *Volcano* grafika, kuriame parodomos visų chromosomų *p* 
vertės ir efekto dydžiai. Žemiau raudonos linijos ir pilka spalva reikšmės yra
laikomos statistiškai nereikšmingomis.

Grafike pavaizduoti raudoni ir geltoni taškai. Kaip ir prieš tai pavaizduotame 
*volcano* grafike, raudoni taškai nėra laikomi statistiškai patikimais ir 
oranžiniai yra laikomi ir yra tinkami tolimesnėms analizėms.

Taip pat iš grafiko matome, kad tarp grupių nera labai didelio statistinio 
skirtumo, kadangi didžioji dalis reikšmių yra išsidesčiusi ties 0 *x* ašyje.
...
p4 <- ggplot(df, aes(x = Effect_size, y = -log10(P), fill = significance,
        ...
        labs(title = "P and effect size values of all chromosomes",
             x = "Effect size", y = "-log10(P)") +
        scale_x_continuous(breaks = c(seq(-0.02, 0, 0.02)),
                           limits = c(-0.02, 0.02)) +
        theme(axis.text.x = element_text(vjust = 0.5, size = 8,
                                         colour = "black"),
              plot.title = element_text(hjust = 0.5, face = "bold"),
