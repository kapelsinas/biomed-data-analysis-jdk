...
Žemiau vaizduojamas grafikas, kuriame vizualizuojamos visų chromosomų *p*
vertės ir efekto dydžiai. Žemiau raudonos linijos pilka spalva
yra atvaizduojamos visos *p* vertės, kurias logaritmavus, jos buvo mažesnės
už *-log(0.05)* reikšmę. Visos šios reikšmės yra laikomos statistiškai
nereikšmingomis.

Tamsiai raudona spalva pavaizduoti taškai nėra laikomi statistiškai
patikimais, nepaisant to, jog logaritmuotos *p* vertės yra didesnės už
*-log(0.05)*. Taip yra todėl, nes šiuo atveju reikia atsižvelgti ir į
efekto dydžio reikšmę. Reikšmės laikomos patikimomis tada, kai efekto dydžio
reikšmė viršija 0.2 ir *p* reikšmė viršija *-log(0.05)* reikšmę.

Grafike yra pavaizduoti oranžiniai taškai, kurie rodo statistiškai patikimas
reikšmes. Tai reiškia, jog atsižvelgus į efekto dydžius ir logaritmuotas
*p* vertes, oranžine spalva pažymėti taškai yra reikšmingi ir gali būti
panaudoti tolimesnėms analizėms.
...
- Nuliniame modelyje kaip pašaliniai efektai buvo pasirinkti aspirino vartojimas 
ir polipų buvimas.
- Pirmajame modelyje pašaliniais efektais buvo amžius, aspirino vartojimas ir
polipų buvimas.
...
Antroje, ketvirtoje ir penktoje pozicijose matome, kad tiek aspiriną
vartojančių, tiek nevartojančių modifikacijos įverčiai yra vienodi ir yra ties 0.
...
## 4.2. Daugkartinio testavimo *p* verčių korekcija, kontroliuojant pašalinius
## efektus.
...
Žemiau pateiktame pateiktame *Volcano* grafike 
```{r lm_Chromosome_volcano_plot, echo=FALSE, message=FALSE, warning=FALSE}
df <- data.frame(P = as.numeric(p_effects$P),
                 Effect_size = as.numeric(p_effects$Effect))
df <- df %>%
  mutate(significance = case_when(Effect_size > 0 & P <= 0.05 ~ "up",
...
p4 <- ggplot(df, aes(x = Effect_size, y = -log10(P), fill = significance,
             alpha = significance)) + 
        geom_point(shape = 21,color='transparent',  alpha = 0.8) + 
        geom_hline(yintercept = -log10(0.05), linetype = "dashed", size = 0.7,
                   color = "red") + 
        geom_vline(xintercept = c(log2(0.5), log2(2)), linetype = "dashed") +
        annotate("text", x = 0.4, y = 3, label = stringr::str_wrap("Line
...
## 4.3. Gene Ontology Enrichment analizė kontroliuojant pašalinius efektus.

Pasinaudojus ***GOrilla*** įrankiu buvo analizuojami genai esantys konkrečiose
genominėse pozicijose.
...
```{r lm_GO_analysis_all_genes, warning=FALSE, include=FALSE}
# Extracting 'UCSC_RefGene_Name' column values for the entire positions:
bgenes <- c(unique(annot_row$UCSC_RefGene_Name)) %>%
               stri_remove_empty(., na_empty = FALSE)
...
Kaip ir prieš tai atlikus *Gene Ontology Enrichment* analizę su ***GOrilla*** 
įrankiu, genai buvo nagrinėjami pagal tris kategorijas:
...
Žemiau pateikiamoje lentelėje nurodomi 10 funkcijų, kurias atlieka pasirinkti 
genai :
...
Galime pamatyti kad didžioji dalis genų atlieka prisijungimo funkciją. Taip pat
matome, kad labai stipriai išsiskyria genai, kurie atlieka baltymų prisijungimo
funkcija.
...
print(GO_comp)
```
Iš lentelėje matomų biologinių komponentų galime pamatyti, kad didžioji dalis 
genų yra iš lastelės sandaros dalių, tokių kaip membrana, nucleoplazmą.

Apibendrinant lentesė pateiktus duomenis kontroliuojant pašalinius efektus, 
galima padaryti išvadą, kad didžioji dalis genų dalyvauja **anatomijos**,
**organų** ir **lastelės sandaros dalių** formavime.
