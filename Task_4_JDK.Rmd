...
```{r Chromosome_volcano_plot, fig.align="center", fig.height=3, fig.width=4, echo=FALSE, message=FALSE, warning=FALSE}
...
patikimais. Oranžine spalva pažymėti taškai yra laikomi patikimais ir
yra tinkami tolimesnėms analizėms.
...
GO_proc <- read.csv("lm_gene_process.csv")
GO_proc <- GO_proc[order(GO_proc[, "P.value"]), , drop = FALSE] %>%
              as.data.table() %>%
              as_huxtable() %>%
              set_bold(1, everywhere, TRUE) %>%
              set_all_borders('NA') %>%
...
Žemiau pateikiamoje lentelėje nurodomos 10 funkcijų, kurias atlieka pasirinkti
genai :

```{r lm_GO_analysis_Function, echo=FALSE, message=FALSE, warning=FALSE}
GO_func <- read.csv("lm_gene_function.csv")
GO_func <- GO_func[order(GO_func[, "P_value"]), , drop = FALSE] %>%
              as.data.table() %>%
              as_huxtable() %>%
              set_bold(1, everywhere, TRUE) %>%
...
pat matoma, kad labai stipriai išsiskiria genai, kurie atlieka **baltymų
prisijungimo funkciją**.

#### Biologiniai komponentai

Žemiau pateikiamoje lentelėje nurodomi 10 biologinių komponentų, kuriuose
nustatomi atrinktų genų produktai:

```{r lm_GO_analysis_Component, echo=FALSE, message=FALSE, warning=FALSE}
GO_comp <- read.csv("lm_gene_component.csv")
GO_comp <- GO_comp[order(GO_comp[, "P_value"]), , drop = FALSE] %>%
              as.data.table() %>%
              as_huxtable() %>%
              set_bold(1, everywhere, TRUE) %>%
...
didžioji dalis genų dalyvauja **anatomijos**, **organų** ir **ląstelės sandaros
dalių** formavime.
