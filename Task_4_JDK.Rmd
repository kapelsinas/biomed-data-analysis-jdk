...
# T.test pradžia. Reikalingas trumpas paaiškinimas, kodėl jį atliekam ir ką
# jis duoda (aiškinti kaip medikui, o ne kaip dėstytojui).

```{r T_test_performance}
t_test <- as.data.frame(apply(beta_nonusers, 2, function(beta_nonusers) 
             t.test(beta_nonusers, beta_users)$p.value)) %>%
                   setNames(., c("pvalue"))
```

# Efekto dydžio skaičiavimo pradžia. Truputį aprašyti, kas tas efekto dydis,
