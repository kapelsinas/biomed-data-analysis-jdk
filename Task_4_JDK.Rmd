...
biopsijos, buvo gauta mėginius aprašanti lentelė, kurią peržiūrėjus buvo galima
sudaryti grupes pagal kelis požymius:
...
turinčių žemiausias *p* vertes. Kiekviename atskirame grafike yra vaizduojami
aspirino nevartojančių ir aspiriną vartojančių moterų modifikacijos įverčiai
...
kad analizuojamoje pozicijoje mažų modifikacijos įverčių nėra, tačiau
modifikacijos įverčių yra daugiau ties didesnėmis modifikacijos reikšmėmis.
...
### *Manhattan* grafikas *7* chromosomos *p* vertėms atvaizduoti

REIKIA APRAŠYMO...

```{r Chromosome_manhattan_plot, echo=FALSE, warning=FALSE}
chr <- annot_row[annot_row$chr == "chr7", ]$Name
bps <- annot_row[annot_row$chr == "chr7", ]$pos
pvals <- p_effects[p_effects$position %in% chr, "P"]

chr_p_effects <- data.frame(CHR = rep(7, length(chr)), BP = as.numeric(bps),
                            P = -log10(as.numeric(pvals)))
chr_p_effects

chr_p_effects <- chr_p_effects %>%
  mutate(significance = case_when(P <= 1 ~ "up", P > 1 ~ "down"))
 
cols <- c("up" = "#ffdc81", "down" = "#7e0a23") 
sizes <- c("up" = 0.2, "down" = 0.2) 
alphas <- c("up" = 1, "down" = 1)

#p3 <- ggplot(df, aes(x = Effect_size, y = -log10(P), fill = significance,
#             alpha = significance)) + 

p3 <- ggplot(chr_p_effects, aes(x = BP, y = P, size = P)) +
  geom_point(alpha = 0.75, size = 0.4) +
  labs(title = "7th chromosome P value Manhattan plot", x = "Position",
       y = "-log10(P)") +
  ylim(c(0, 10)) +
  theme_minimal() +
  theme(axis.text.x = element_text(vjust = 0.5, size = 8, colour = "black"),
              plot.title = element_text(hjust = 0.5, face = "bold"),
              plot.subtitle = element_text(hjust = 0.5, face = "bold"),
...
pritaikytos tam, jog būtų sumažinta riba, pagal kurią galima spręsti apie
vidurkių reikšmingumą (sumažėtų galimas ***false positive*** reikšmių
procentas), kai tas pats testas yra atliekamas daug kartų.

Žemiau pateiktoje lentelėje nurodomi reikšmingų pozicijų skaičius, neatlikus *p*
...
```{r T_test_effect_size_side_effects, message=FALSE, warning=FALSE, include=FALSE}
