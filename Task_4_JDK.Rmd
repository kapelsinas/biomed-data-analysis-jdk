...
knitr::opts_chunk$set(fig.height = 13, fig.width = 16, fig.pos = 'H')
```
...
-   Moterų, kurioms yra nuo *50* iki *65* metų, grupė ir moterų, kurioms yra nuo
    *65* iki *80* metų, grupė;
...
mėginių sudarė kievieną grupę (aspirino nevartojančių moterų ir aspiriną
vartojančių moterų) prieš atliekant antrąją praktinę užduotį, kai buvo turimi
*124* mėginiai, bei diagramos stulpeliai, kuriuose vaizduojami mėginių kiekiai
grupėse po mėginių pašalinimo (likus *96* mėginiams):

```{r Aspirin_user/nonuser_selection, fig.align="center", fig.height=3, fig.width=4, message=FALSE, warning=FALSE, echo=FALSE}
...
svarbiu, statistikoje taikomu ***t-testu***, kuris tikrina, ar modifikacijos
reikšmių vidurkiai skirtingose DNR pozicijose sutampa.
...
nevartojimas įtakoja DNR modifikacijų reikšmes ir ar vidurkių skirtumas tarp
grupių yra statistiškai reikšmingas.
...
```{r P_value_histogram, echo=FALSE, message=FALSE, warning=FALSE, fig.align="center", fig.height=3, fig.width=5}
...
```{r Min_p_value_visualization, fig.align="center", fig.height=3, fig.width=5, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r Chromosome_manhattan_plot, fig.align="center", fig.height=3, fig.width=5, echo=FALSE, message=FALSE, warning=FALSE}
# Extracting position names, position numbers and p values of the 8th chr:
...
### *Volcano* grafikas visų chromosomų efektų dydžiams ir *p* vertėms atvaizduoti

Žemiau vaizduojamas grafikas, kuriame vizualizuojamos visų chromosomų *p* vertės
ir efekto dydžiai. Žemiau raudonos linijos pilka spalva yra atvaizduojamos visos
*p* vertės, kurias logaritmavus, jos buvo mažesnės už *-log(0.05)* reikšmę.
Visos šios reikšmės yra laikomos statistiškai nereikšmingomis.

Tamsiai raudona spalva pavaizduoti taškai nėra laikomi statistiškai patikimais,
nepaisant to, jog logaritmuotos *p* vertės yra didesnės už *-log(0.05)*. Taip
yra todėl, nes šiuo atveju reikia atsižvelgti ir į efekto dydžio reikšmę.
Reikšmės laikomos patikimomis tada, kai efekto dydžio reikšmė viršija *0.2* ir
*p* reikšmė viršija *-log(0.05)* reikšmę.

Grafike yra pavaizduoti oranžiniai taškai, kurie rodo statistiškai patikimas
reikšmes. Tai reiškia, jog atsižvelgus į efekto dydžius ir logaritmuotas *p*
vertes, oranžine spalva pažymėti taškai yra reikšmingi ir gali būti panaudoti
tolimesnėms analizėms.

```{r Chromosome_volcano_plot, fig.align="center", echo=FALSE, message=FALSE, warning=FALSE}
# Forming a dataframe that will be used for making a Volcano plot:
df <- data.frame(P = as.numeric(p_effects$P),
                 Effect_size = as.numeric(p_effects$Effect))

# Defining when effect sizes and P values will be marked as 'up' and 'down':
df <- df %>%
  mutate(significance = case_when(Effect_size > 0.2 & P <= 0.05 ~ "up",
                                  Effect_size <= 0.2 & P <= 0.05 ~ "down",
                                  TRUE ~ "ns"))   

# Defining colors, sizes and alpha values for the 'up' and 'down' numbers:
cols <- c("up" = "#f78e0e", "down" = "#7e0a23", "ns" = "grey") 
sizes <- c("up" = 0.5, "down" = 0.5, "ns" = 0.1) 
alphas <- c("up" = 1, "down" = 1, "ns" = 0.5)

p4 <- ggplot(df, aes(x = Effect_size, y = -log10(P), fill = significance,
             alpha = significance)) + 
        geom_point(shape = 21, color = "transparent", alpha = 0.7) + 
        geom_hline(yintercept = -log10(0.05), linetype = "dashed", size = 0.7,
                   color = "red") + 
        geom_vline(xintercept = c(log2(0.5), log2(2)), linetype = "dashed") +
        annotate("text", x = 0.18, y = 0.2, label = stringr::str_wrap("Line
                 indicating where values lose significance considering effect
                 sizes and P values", width = 40),
                 size = 2, color = "red", fontface = 4) +
        scale_fill_manual(values = cols) +
        scale_size_manual(values = sizes) +
        scale_alpha_manual(values = alphas) +
        labs(title = stringr::str_wrap("P and effect size values of all chromosomes", width = 40),
             x = "Effect size", y = "-log10(P)") +
        scale_x_continuous(breaks = round(seq(-0.3, 0.3, by = 0.1), 3),
                           limits = c(-0.3, 0.3)) +
        scale_y_continuous(breaks = seq(-5, 11, by = 2)) +
...
hipotezę. Šioje praktinės užduoties dalyje buvo atliktos *p* reikšmių
korekcijos, kurios pritaikytos tam, jog bendra klaidos tikimybė, kartojant testą
daug kartų, būtų mažesnė arba lygi dažniausiai naudojamai *p* reikšmei *(0.05).*
...
GO_proc <- read.csv("GOrilla/gene_process.csv")
GO_proc <- GO_proc[order(GO_proc[, "P_value"]), , drop = FALSE] %>%
              as_huxtable() %>%
              set_bold(1, 1:3, TRUE) %>%
              set_all_borders(1) %>%
              set_background_color(1:1, 1:3 , "#ffbb5c") %>%
              set_align(1, 1, "center")
...
GO_func <- read.csv("GOrilla/gene_function.csv")
...
Žemiau pateikiamoje lentelėje išskiriami 6 biologiniai komponentai, kuriuose nustatomi atrinktų genų produktai:

```{r GO_analysis_Component, echo=FALSE, message=FALSE, warning=FALSE}
GO_comp <- read.csv("GOrilla/gene_component.csv")
...
Apibendrinus lentelėse pateiktus duomenis bei pilnus ***GOrilla*** analizės
...
Ketvirtoje užduoties dalyje statistinis hipotezių testavimas buvo atliktas,
naudojant tiesinius modelius, kontroliuojant pašalinius efektus:

-   **Nuliniame modelyje** kaip pašaliniai efektai buvo pasirinkti **aspirino
    vartojimas** ir **polipų buvimas**.
-   **Pirmajame modelyje** pašaliniais efektais buvo **amžius**, **aspirino
    vartojimas** ir **polipų buvimas**.

***(P vertės)*** buvo gautos iš antrojo tiesinio modelio amžiaus koeficiento.
***Efekto dydis*** buvo gautas, naudojant dispersinę analizę, analizuojant
vidurkių skirtumą tarp nulinio ir pirmojo modelio.

```{r Anova_test, warning=FALSE, include=FALSE}
pvalues <- matrix(NA, nrow = nrow(beta), ncol = 2)
effect_sizes <- matrix(NA, nrow = nrow(beta), ncol = 2)

# Using linear models to create two models to control "outsider" effects.
# Using linear model "mod1" age coefficient to get effect_size and Anova table
# from mod0 and mod1 to get pvalue:
for (row in 1:nrow(beta)) {
...
papildomus pašalinius efektus. Šiuo atveju pasirinkti kontroliuojami pašaliniai
efektai buvo aspirino vartojimas ir polipų buvimas.

Kaip ir prieš tai buvusioje *p* verčių paskirstymo histogramoje buvo gautas
ganėtinai didelis skaičius *p* verčių, kurios yra mažesnės už *0.05*.

```{r lm_P_value_histogram, echo=FALSE, fig.align="center", fig.height=3, fig.width=5, warning=FALSE}
...
p1 <- ggplot(p_df, aes(x = pvals)) + 
        ...
        labs(x = "P value", y = "Count",
             title = "P-value distribution histogram") +
        scale_x_continuous(breaks = seq(0, 1, by = 0.05)) +
        annotate("text", x = 0.2, y = 50000, label = stringr::str_wrap("Line
                 indicating 0.05 p value break", width = 20), size = 3,
                 color = "red", fontface = 4) +
        theme(axis.text.x = element_text(angle = 45, vjust = 0.5,
                                         colour = "black"),
...
nevartojančių moterų grupėje esama mažesnių modifikacijos įverčių. Taip pat
galima pastebėti, kad vartojančių aspiriną moterų grupėje mažų įverčių yra
mažiau.

Antroje, ketvirtoje ir penktoje pozicijose matoma, kad tiek aspiriną
vartojančių, tiek nevartojančių modifikacijos įverčiai yra vienodi ir yra ties
*0*.

Trečioje pozicijoje ***(cg02934525)*** įverčiai yra gan panašūs tiek vartojančių
aspiriną, tiek aspirino nevartojančių moterų grupėse.
...
```{r lm_Chromosome_manhattan_plot, fig.align="center", fig.height=3, fig.width=5, echo=FALSE, warning=FALSE}
p_effects <- data.frame(position = pvalues$Position, P = pvalues$`P-value`,
                        Effect = effect_sizes$`Effect size`)
...
Žemiau vaizduojamas *Volcano* grafikas, kuriame parodomos visų chromosomų *p*
vertės ir efekto dydžiai. Žemiau raudonos linijos esančios ir pilka spalva
pavaizduotos reikšmės yra laikomos statistiškai nereikšmingomis.

Grafike pavaizduoti raudoni ir geltoni taškai. Kaip ir anksčiau analizėje
pavaizduotame *Volcano* grafike, raudoni taškai nėra laikomi statistiškai
patikimais. Oranžine spalva pažymėti taškai yra laikomi patikimais ir yra
tinkami tolimesnėms analizėms.

Taip pat iš grafiko matoma, kad tarp grupių nėra labai didelio statistinio
skirtumo, kadangi didžioji dalis reikšmių yra išsidėsčiusi ties *0* *x* ašyje.

```{r Chromosome_volcano_plot_lm, echo=FALSE, message=FALSE, warning=FALSE}
df <- data.frame(P = as.numeric(p_effects$P),
                 Effect_size = as.numeric(p_effects$Effect))

df <- df %>%
  mutate(significance = case_when(Effect_size > 0.005 & P <= 0.05 ~ "up",
...
p4 <- ggplot(df, aes(x = Effect_size, y = -log10(P), fill = significance,
        ...
        labs(title = "P and effect size values of all chromosomes",
             x = "Effect size", y = "-log10(P)") +
        scale_x_continuous(breaks = c(seq(-0.05, 0, 0.05)),
                           limits = c(-0.05, 0.05)) +
        theme(axis.text.x = element_text(vjust = 0.5, size = 8,
                                         colour = "black"),
              plot.title = element_text(hjust = 0.5, face = "bold"),
...
## 4.2. Daugkartinio testavimo *p* verčių korekcija, kontroliuojant pašalinius efektus

Žemiau pateiktoje lentelėje nurodomi reikšmingų pozicijų skaičiai, neatlikus *p*
verčių korekcijos bei pritaikius ***FDR*** ir ***Bonferroni*** korekcijas:

```{r lm_P_value_corrections, echo=FALSE, message=FALSE, warning=FALSE}
fdr <- p.adjust(pvalues$`P-value`, method = "fdr",
                n = length(pvalues$`P-value`))
bf <- p.adjust(pvalues$`P-value`, method = "bonferroni",
               n = length(pvalues$`P-value`))
p_corr <- data.frame(Position = pvalues$Position,
                     No_correction = pvalues$`P-value`,
                     FDR = fdr, Bonferroni = bf)
...
## 4.3. *Gene Ontology Enrichment* analizė
...
```{r lm_GO_analysis_selected_genes_by_p, message=FALSE, warning=FALSE, include=FALSE}
# Extracting 'UCSC_RefGene_Name' column values of positions that have a p value
# which is lower or equal to 0.05 after performing FDR correction:
selected_pos <- p_corr[p_corr$FDR <= 0.05, 1] %>% 
                  annot_row[.,]
...
write.table(fgene_list, file = "lm_foreground_gene_list.txt", quote = FALSE,
            col.names = FALSE, row.names = FALSE)
```

```{r lm_GO_analysis_all_genes, message=FALSE, warning=FALSE, include=FALSE}
# Extracting 'UCSC_RefGene_Name' column values for the entire positions:
bgenes <- c(unique(annot_row$UCSC_RefGene_Name)) %>%
               stri_remove_empty(., na_empty = FALSE)
...
write.table(bgene_list, file = "lm_background_gene_list.txt", quote = FALSE,
            col.names = FALSE, row.names = FALSE)
```

#### Procesai\ 

Žemiau pateikiamoje lentelėje nurodomi 10 procesų, kuriuose dalyvauja atrinkti genai:

```{r lm_GO_analysis_Process, echo=FALSE, message=FALSE, warning=FALSE}
GO_proc <- read.csv("GOrilla/lm_gene_process.csv")
GO_proc <- GO_proc[order(GO_proc[, "P_value"]), , drop = FALSE] %>%
              as_huxtable() %>%
              set_bold(1, 1:3, TRUE) %>%
              set_all_borders(1) %>%
...
Lentelėje pateikti duomenys rodo, kad atrinktuose genuose kaip ir prieš tai
atliktoje procesų analizėje didžioji dalis genų dalyvauja **anatomijos** ir
**organų formavimosi** procesuose.

\pagebreak

#### Funkcijos\

Žemiau pateikiamoje lentelėje nurodomos 10 funkcijų, kurias atlieka pasirinkti genai :

```{r lm_GO_analysis_Function, echo=FALSE, message=FALSE, warning=FALSE}
GO_func <- read.csv("GOrilla/lm_gene_function.csv")
GO_func <- GO_func[order(GO_func[, "P_value"]), , drop = FALSE] %>%
              as_huxtable() %>%
              set_bold(1, 1:3, TRUE) %>%
              set_all_borders(1) %>%
...
Remiantis lentelėje pateikta informacija apie funkcijas, kurias atlieka genai,
galime pamatyti, kad didžioji dalis genų atlieka **prisijungimo** funkciją. Taip
pat matoma, kad labai stipriai išsiskiria genai, kurie atlieka **baltymų prisijungimo funkciją**.

\pagebreak

#### Biologiniai komponentai\

Žemiau pateikiamoje lentelėje nurodomi 10 biologinių komponentų, kuriuose nustatomi atrinktų genų produktai:

```{r lm_GO_analysis_Component, echo=FALSE, message=FALSE, warning=FALSE}
GO_comp <- read.csv("GOrilla/lm_gene_component.csv")
GO_comp <- GO_comp[order(GO_comp[, "P_value"]), , drop = FALSE] %>%
              as_huxtable() %>%
              set_bold(1, 1:3, TRUE) %>%
              set_all_borders(1) %>%
...
Iš lentelėje išvardintų biologinių komponentų matoma, kad didžioji dalis genų
produktų įeina į ląstelės sandaros dalių, tokių kaip **membrana**,
**nucleoplazma**, sudėtį.

Apibendrinant lentelėse pateiktus duomenis, kurie buvo gauti nagrinėjant
pozicijas, kontroliuojant pašalinius efektus, galima padaryti išvadą, kad
didžioji dalis genų dalyvauja **anatomijos**, **organų** ir **ląstelės sandaros dalių** formavime.
